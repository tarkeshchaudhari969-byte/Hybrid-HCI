...
class Gest(IntEnum):
    ...
    INDEX = 8
    FIRST2 = 12
    FIRST3 = 14    # Index + Middle + Ring fingers
    LAST4 = 15
    THUMB = 16    
    PALM = 31
...
class Controller:
    ...
    prev_hand = None
    pinch_threshold = 0.3
    # Minor-hand-only custom controls
    pinch_start_x = None
    pinch_start_y = None
    last_pinch_action_time = 0
    ...
    palm_start_x = None
    last_tab_time = 0
    last_task_view_time = 0
    task_view_cooldown = 2.0
    ...
    @staticmethod
    def simple_minor_pinch_control(hand_result):
        """
        Minor/left hand pinch control using a neutral center point.

        Pinch starts = saves center point.
        Move up/down from center = continuous scroll.
        Move left/right from center = volume down/up.
        Release pinch = center resets in handle_controls().
        """
        pyautogui.FAILSAFE = False

        # Landmark 9 = palm center, more stable than fingertip
        x = int(hand_result.landmark[9].x * 1000)
        y = int(hand_result.landmark[9].y * 1000)

        # Set neutral/center point once when pinch starts
        if Controller.pinch_start_x is None or Controller.pinch_start_y is None:
            Controller.pinch_start_x = x
            Controller.pinch_start_y = y
            print("Minor pinch center set:", x, y)
            return

        dx = x - Controller.pinch_start_x
        dy = y - Controller.pinch_start_y
        current_time = time.time()

        # Decide whether movement is mainly vertical or horizontal
        vertical_deadzone = 45
        horizontal_deadzone = 70

        # Vertical movement = continuous scroll
        if abs(dy) > abs(dx) and abs(dy) > vertical_deadzone:
            if current_time - Controller.last_pinch_action_time < 0.06:
                return

            if dy < 0:
                pyautogui.scroll(25)
                print("Continuous Scroll Up")
            else:
                pyautogui.scroll(-25)
                print("Continuous Scroll Down")

            Controller.last_pinch_action_time = current_time

        # Horizontal movement = volume control
        elif abs(dx) > abs(dy) and abs(dx) > horizontal_deadzone:
            if current_time - Controller.last_pinch_action_time < 0.15:
                return
    ...
    def screenshot_control():
        ...
        Takes only one screenshot per fist hold.
        """
        current_time = time.time()

        if Controller.screenshot_taken:
            return
    ...
    @staticmethod
    def task_view_control():
        """Peace/V sign = Windows Task View (Win + Tab)."""
        current_time = time.time()

        if current_time - Controller.last_task_view_time < Controller.task_view_cooldown:
            return

        pyautogui.hotkey('win', 'tab')
        print("Task View Opened")

        Controller.last_task_view_time = current_time

    @staticmethod
    def copy_control():
        """Index finger only = Ctrl + C."""
        current_time = time.time()

        if current_time - Controller.last_copy_time < Controller.copy_cooldown:
            return

        pyautogui.hotkey('ctrl', 'c')
        print("Copied")

        Controller.last_copy_time = current_time

    @staticmethod
    def paste_control():
        """Index + Middle + Ring fingers = Ctrl + V."""
        current_time = time.time()

        if current_time - Controller.last_paste_time < Controller.paste_cooldown:
            return

        pyautogui.hotkey('ctrl', 'v')
        print("Pasted")

        Controller.last_paste_time = current_time

    @staticmethod
    def palm_tab_control(hand_result):
        """
        Open palm swipe left/right = browser tab switching.
        Palm appears = saves center x.
        ...
        if Controller.palm_start_x is None:
            Controller.palm_start_x = x
            print("Palm center set:", x)
            return

        current_time = time.time()

        if current_time - Controller.last_tab_time < 0.8:
            return

        dx = x - Controller.palm_start_x
        print("Palm dx:", dx)
        ...
        Controller.last_tab_time = current_time

        # Reset center after one tab switch.
        # This prevents repeated switching just by holding palm at the side.
        Controller.palm_start_x = None
    ...
    def handle_controls(gesture, hand_result):
        """Implements custom minor-hand-only gesture functionality."""

        # If no minor hand is available, reset temporary gesture states
        if hand_result is None:
            Controller.pinch_start_x = None
            Controller.pinch_start_y = None
            Controller.last_pinch_action_time = 0
            Controller.palm_start_x = None
            Controller.screenshot_taken = False
            return

        # Reset pinch center when pinch is released
        ...
        # Allow next screenshot only after fist is released
        if gesture != Gest.FIST:
            Controller.screenshot_taken = False

        # Reset palm swipe center when palm is released
        ...
        if gesture == Gest.PINCH_MINOR:
            ...
            Controller.simple_minor_pinch_control(hand_result)

        # Closed fist = screenshot
        elif gesture == Gest.FIST:
            Controller.screenshot_control()

        # Peace/V sign = Win + Tab Task View
        elif gesture == Gest.V_GEST:
            Controller.task_view_control()

        # Index finger only = Ctrl + C
        elif gesture == Gest.INDEX:
            Controller.copy_control()

        # Index + Middle + Ring fingers = Ctrl + V
        elif gesture == Gest.FIRST3:
            Controller.paste_control()

        # Open palm disabled for now
        elif gesture == Gest.PALM:
            pass

        # Disable all major-hand/cursor/click actions
        ...
        with mp_hands.Hands(max_num_hands = 2,min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                if results.multi_hand_landmarks:                   
                    ...
                    # Use ONLY the minor/left hand for webcam gestures.
                    # Right/major hand is ignored because it has the black ESP32 air-mouse glove.
